...
class Solution:
    ...
    step5  after processing all meetings, size of heap will be the rooms 
        ...
        rooms = []
        # sort by start but put end point into pq.
        heapq.heappush(rooms,intervals[0][1])
        for i in range(1, len(intervals)):
            #  if current start point >= mininum end point, need to remove earliest end room
...
# time O(NlgN) space O(2N)
        """
        1. cache start, end point seperatedly, and sort 
        2. iterate start ptr, each time add one rooms 
        3. each time end ptr smaller than start ptr, room -1, end ptr increment 
        Note, when start ptr == end ptr (ei == sj), room will decrement  
        """
class Solution:
    def minMeetingRooms(self, intervals: List[List[int]]) -> int:
        """
